...
# Enhanced gum_pretty_print function with color parameter
function gum_pretty_print() {
    local message=$1
...
function check_install_homebrew() {
    if ! command_exists brew; then
        gum_pretty_print "Homebrew is not installed. Installing..." red
        install_homebrew
        gum_pretty_print "Homebrew installation completed." green
    fi
}
...
function check_install_gum() {
    if ! command_exists gum; then
        gum_pretty_print "GUM is not installed. Installing..." red
        brew install gum
        gum_pretty_print "GUM installation completed." green
    fi
}
...
function install_item() {
    ...
    if command_exists "$item" || is_installed "$item" "$type"; then
        gum_pretty_print "$item is already installed." $COLOR_YELLOW
        return 0
    fi

    if brew install $type "$item"; then
        gum_pretty_print "Installed $item successfully." $COLOR_GREEN
    else
        gum_pretty_print "Failed to install $item." $COLOR_RED
        return 1
    fi
}
...
function install_items() {
    ...
    if [ ${#items[@]} -ne 0 ]; then
        gum_pretty_print "Installing $item_type..." $COLOR_BLUE
        for item in "${items[@]}"; do
            install_item "$item" "$type"
        done
    else
        gum_pretty_print "No $item_type to install." $COLOR_YELLOW
    fi
}
...
function uninstall_item() {
    ...
    if is_installed "$item" "$type"; then
        ...
        gum_pretty_print "Uninstalled $item." $COLOR_GREEN
    else
        gum_pretty_print "$item is not installed." $COLOR_YELLOW
    fi
}
...
function uninstall_items() {
    ...
    if [ ${#items[@]} -ne 0 ]; then
        gum_pretty_print "Checking and uninstalling $item_type..." $COLOR_BLUE
        for item in "${items[@]}"; do
            uninstall_item "$item" "$type"
        done
    else
        gum_pretty_print "No $item_type to uninstall." $COLOR_YELLOW
    fi
}
...
function bootstrap_main() {
    ...
    case "$ACTION" in
        Setup)
            gum_pretty_print "Installing apps..." $COLOR_BLUE
            install_apps
            ;;
        Teardown)
            gum_pretty_print "Uninstalling apps..." $COLOR_BLUE
            uninstall_apps
            ;;
        *)
            gum_pretty_print "Invalid option, exiting..." $COLOR_RED
            exit 1
            ;;
    esac
}
